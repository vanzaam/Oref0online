...
main() {
 ...
 PersistentBTPAN="false"
	...
	if ifconfig | egrep -q "wlan0" >/dev/null; then
        ...
        echo -n "At $(date) my Internet status is: "
...
function check_online {
    if ! ping 8.8.8.8 -c 1 | grep time= >/dev/null; then
        echo offline
        return 1
    else
        echo online
...
function bt_connect {
    ...
    for MAC; do
        #echo -n "At $(date) my Network status is: "
        if [ "$PersistentBTPAN" == "true" ]; then
            if has_ip bnep0; then
                break
            fi
...
function stop_hotspot {
    if grep -q $HostAPDIP /etc/network/interfaces || iwconfig wlan0 | grep Mode:Master; then
        ...
        if ! cat /root/myopenaps/preferences.json | jq -e .offline_hotspot >/dev/null; then
...
function start_hotspot {
    ...
    if ls /tmp/disable_hotspot; then
        ...
        || ! cat /root/myopenaps/preferences.json | jq -e .offline_hotspot >/dev/null; then
